/**
 * Customer Forgot Password Reset Confirmation API Route (Proxy)
 *
 * POST /api/verify/customer-reset-password
 */
...
function logRequest(data: ResetPasswordRequest, ip: string) {
  const logData = {
    timestamp: new Date().toISOString(),
    type: 'PROXY_REQUEST',
    endpoint: '/api/verify/customer-reset-password',
    org: data.org,
    token: data.token,
...
function logError(error: unknown, data: Partial<ResetPasswordRequest>) {
  const logData = {
    timestamp: new Date().toISOString(),
    type: 'PROXY_ERROR',
    endpoint: '/api/verify/customer-reset-password',
...
export async function POST(request: NextRequest) {
  ...
  let requestData: Partial<ResetPasswordRequest> = {};
  ...
  try {
    ...
    // Support both 'username' and 'userName' from client to be safe
    ...
    const { org, token, userName, password, customerId } = validatedData;

    const baseUrl = (EXTERNAL_API_URL || '').replace(/\/$/, '');
    
    const externalUrl = `${baseUrl}/api/Registration/org/${org}/action/ConfirmCustomerForgotPasswordReset/${token}/${customerId}`;
    ...
    console.log('\n========================================');
    console.log('🚀 EXTERNAL API CALL - CUSTOMER RESET PASSWORD');
    console.log('========================================');
    ...
    console.log('\n========================================');
    console.log('📥 EXTERNAL API RESPONSE - CUSTOMER RESET PASSWORD');
    console.log('========================================');
    ...
    const apiLog = {
      ...
      method: 'POST',
      url: externalUrl,
      path: '/api/verify/customer-reset-password',
    ...
    if (response.status >= 200 && response.status < 300) {
      ...
      if (!responseStatus || responseStatus.toUpperCase() === 'OK' || responseStatus.toUpperCase() === 'SUCCESS') {
        return NextResponse.json(
          {
            success: true,
            data: response.data,
